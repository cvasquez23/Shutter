<?php
/**
   * The footer for Giem Photography
   *
   * @package Giem Photography
   * @since Giem Photography Theme 1.0
**/
?>

      <!-- Footer -->
      <footer class="tag">
      <div class="container">
         <div class="row justify-content-center">
            <a class="back-to-top" href="#home"><i class="fas fa-chevron-up fa-2x fa-up"></i></a><br>
         </div>
         <div class="row justify-content-center">
            <a class="back-to-top" href="#home">Back to top</a>
         </div>
         <div class="row social-footer justify-content-center">
            <a href="https://www.facebook.com/pg/Kim-Zuidema-Giems-Photography-392580977567305/services/" target="_blank"><i class="fab fa-facebook-f fa-2x"></i></a>
            <a href="" target="_blank"><i class="fab fa-twitter fa-2x"></i></a>
            <a href="" target="_blank"><i class="fab fa-pinterest-p fa-2x"></i></a>
            <a href="" target="_blank"><i class="fab fa-instagram fa-2x"></i></a>
         </div>
         <div class="row justify-content-center credit">
            <p>&copy <?php echo date('Y'); ?> Giem Photography</p>
         </div>
         <div class="row justify-content-center credit">
            <p>Developed by
            <span class="cv">
               <a href="https://www.chrisvasquez.dev" target="_blank">Chris Vasquez</a>&nbsp
            </span>
            </p>
         </div>
      </div>
      </footer>

      <?php wp_footer(); ?>
   </body>
</html>